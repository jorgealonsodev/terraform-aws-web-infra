...
Last updated: 2026-05-16 (refreshed with compact rules for codex skills)
...
- Trigger: Generate or edit raster images — photos, illustrations, textures, sprites, mockups, transparent-background cutouts. Do NOT use for SVG/vector, icon/logo systems, or HTML/CSS/canvas visuals.
- Rules:
  - Prefer built-in `image_gen` tool; only fall back to CLI `scripts/image_gen.py` when user explicitly requests it or transparency requires `gpt-image-1.5`
  - Never silently switch from built-in to CLI mode
  - For transparency: try chroma-key removal first via `remove_chroma_key.py`; ask before downgrading to `gpt-image-1.5`
  - Save project assets to workspace; never leave them only at default `$CODEX_HOME/*`
...
- Trigger: OpenAI products/APIs documentation, model selection, model upgrade, or prompt-upgrade guidance.
- Rules:
  - Prioritize MCP `openaiDeveloperDocs` tools: `search_openai_docs` → `fetch_openai_doc`
  - For latest model guidance: fetch `https://developers.openai.com/api/docs/guides/latest-model.md` first
  - Preserve explicit user target model requests — do not auto-upgrade to newer models
  - If MCP server missing: run `codex mcp add openaiDeveloperDocs --url https://developers.openai.com/mcp`
  - Fallback: bundled references in `references/`, then official OpenAI domains only
...
- Trigger: Create Codex plugin directories with `.codex-plugin/plugin.json`. NOT for general project scaffolding.
- Rules:
  - Run `scripts/create_basic_plugin.py <name>` from repo root
  - Creates `<plugin-name>/.codex-plugin/plugin.json` with `[TODO]` placeholders
  - Use `--with-marketplace` to update `.agents/plugins/marketplace.json`
  - Use `--with-skills`, `--with-hooks`, etc. for optional companion folders
  - Always ask repo-local vs home-local before generating marketplace entries
...
- Trigger: Install Codex skills from openai/skills curated list or GitHub repos. NOT for general package installation.
- Rules:
  - List: `scripts/list-skills.py`; Install: `scripts/install-skill-from-github.py`
  - Default source: `https://github.com/openai/skills/tree/main/skills/.curated`
  - Installs into `$CODEX_HOME/skills/<skill-name>` (default `~/.codex/skills`)
  - After install: "Restart Codex to pick up new skills"
  - Requires network — request sandbox escalation when running scripts
